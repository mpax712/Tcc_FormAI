...
class ActivityController extends Controller
{
    ...
    public function show(Request $request, Activity $activity): View
    {
        ...
        $canAnswer = $activity->deadline_at->isFuture() || ($submission->reopened_until?->isFuture() ?? false);
        abort_if($submission->status === SubmissionStatus::Draft && ! $canAnswer, 403, 'O prazo desta atividade terminou.');
        $activity->load('questions'); $submission->load('answers');
    ...
    public function save(Request $request, Submission $submission, ActivityQuestion $question, SaveAnswerAction $action): JsonResponse
    {
        ...
        try { $answer = $action->execute($submission, $question, $data); } catch (DomainException $e) { return response()->json(['message' => $e->getMessage()], 409); }
    ...
    {
        $this->authorize('update', $submission);
        try { $action->execute($submission); } catch (DomainException $e) { return back()->withErrors(['submission' => $e->getMessage()]); }
        Cache::forget('dashboard:student:'.$submission->student_id);
        Cache::forget('dashboard:teacher:'.$submission->activity->teacher_id);
        return redirect()->route('student.activities.index')->with('status', 'Atividade enviada. Aguarde a publicacao do professor.');
    }
